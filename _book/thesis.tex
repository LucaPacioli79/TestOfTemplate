...
% Following line changed by Dennis Grobe to remove empty pages after the end of a chapter
% Original state: \documentclass[12pt,twoside]{reedthesis}
...
%added by Dennis Grobe
%pandoc update requires following chunk:
...
\NormalTok{flights\_path <{-}}\StringTok{ }\NormalTok{here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"flights.csv"}\NormalTok{)}
\CommentTok{\# string columns will be read in as strings and not factors now}
\NormalTok{flights <{-}}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(flights\_path, }\DataTypeTok{stringsAsFactors =} \OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{max\_delays <{-}}\StringTok{ }\NormalTok{flights2 }\OperatorTok{\%>\%}
\StringTok{  }\KeywordTok{group\_by}\NormalTok{(carrier\_name) }\OperatorTok{\%>\%}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mean\_delay\_by\_carrier <{-}}\StringTok{ }\NormalTok{flights }\OperatorTok{\%>\%}
\StringTok{  }\KeywordTok{group\_by}\NormalTok{(carrier) }\OperatorTok{\%>\%}
